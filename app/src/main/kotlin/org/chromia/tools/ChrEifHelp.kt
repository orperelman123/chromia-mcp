...
object ChrEifHelp {
    ...
    fun notes(): String = """
        ...
        Official BUILD cli/commands/eif ($EIF_INDEX_URL 307 $EIF_INDEX_URL_SLASH 200 $EIF_INDEX_TITLE): intro Usage chr eif generate-events-config Generate solidity events that EIF will listen to Options --abi --events --target --format Query-only WRITE SKIP skip signed txs sample admin pubkey keygen mnemonic no sample keys no invented 64-hex keygen do not invent flags verbs do not document chr tx signed send keygen samples.
        Official GET-STARTED get-started/use-cases/cross-chain/iccf INDEX ($GET_STARTED_ICCF_USE_CASE_INDEX_URL 307 $GET_STARTED_ICCF_USE_CASE_INDEX_URL_SLASH 200 $GET_STARTED_ICCF_USE_CASE_INDEX_TITLE): slash title WRITE SKIP HELP ONLY Origin parked Query-only skip signed txs no sample keys no invented 64-hex BRIDs no keygen no sign recipe.
        Official ECOSYSTEM ecosystem/extensions INDEX ($ECOSYSTEM_EXTENSIONS_INDEX_URL 307 $ECOSYSTEM_EXTENSIONS_INDEX_URL_SLASH 200 H1 $ECOSYSTEM_EXTENSIONS_INDEX_TITLE). Query-only.
        Official ECOSYSTEM ecosystem/bridge/mass-exit/setup INDEX ($ECOSYSTEM_MASS_EXIT_SETUP_INDEX_URL 307 $ECOSYSTEM_MASS_EXIT_SETUP_INDEX_URL_SLASH 200 H1 $ECOSYSTEM_MASS_EXIT_SETUP_INDEX_TITLE). Query-only. HELP ONLY WRITE SKIP.
        Official ECOSYSTEM ecosystem/governance/getting-started/extensions/eif INDEX ($ECOSYSTEM_GOV_EIF_INDEX_URL 307 $ECOSYSTEM_GOV_EIF_INDEX_URL_SLASH 200 H1 $ECOSYSTEM_GOV_EIF_INDEX_TITLE). Query-only. HELP ONLY WRITE SKIP.
        Official LEARN courses/book-review/setup INDEX ($LEARN_BOOK_REVIEW_SETUP_INDEX_URL 301 $LEARN_BOOK_REVIEW_SETUP_INDEX_URL_SLASH 200 H1 $LEARN_BOOK_REVIEW_SETUP_INDEX_TITLE HELP ONLY WRITE SKIP). Query-only. HELP ONLY WRITE SKIP.
        Official LEARN courses/marketplace-course/module-nft/nft INDEX ($LEARN_MARKETPLACE_NFT_MODEL_INDEX_URL 301 $LEARN_MARKETPLACE_NFT_MODEL_INDEX_URL_SLASH 200 H1 $LEARN_MARKETPLACE_NFT_MODEL_INDEX_TITLE HELP ONLY WRITE SKIP). Query-only. HELP ONLY WRITE SKIP.
    """.trimIndent()
