# Chromia stack expert (Claude Code)

Standalone Claude Code brief for this repository. Codex / generic agents use `AGENTS.md` (same authority and pins).
...
When answering, prefer the `fetch_docs` MCP tool (or ChatGPT-compatible `search` / `fetch`) against this server's RAG store, then confirm versions against GitLab tags. Do not invent APIs, module names, or `chromia.yml` keys.

If Chromia MCP is not already configured in Claude Code:

```bash
claude mcp add chromia --scope user -- java -jar /opt/chromia/chromia-mcp-server.jar
```

Local SSE: `http://127.0.0.1:3001`. Production: `https://mcp.chromia.dev/sse`.
...
- A Docker image that already bundles Claude Code + `chr` + this MCP lives in `claude-code-chromia/`.
...
- Runtime loads local `embeddings.json` first (`CHROMIA_EMBEDDINGS_PATH`, else the first existing of `build/embeddings.json` or `app/build/embeddings.json` relative to cwd, so `java -jar` from the repo root finds the Gradle file). First persist with no env prefers `app/build/embeddings.json` when cwd is the repo root and `app/build/` exists. Then the on-disk cache `$CHROMIA_MCP_HOME/embeddings.bin` + `embeddings.cache.json` (default `~/.chromia-mcp/`; the downloaded JSON is re-encoded once into `EmbeddingStoreBinary` - 0.65 s to read vs 5-11 s for the JSON, which was the whole boot; reused while younger than 7 days, refreshed after, still served if the refresh fails; `CHROMIA_EMBEDDINGS_CACHE=off` disables it). Then, only if neither exists: `CHROMIA_EMBEDDINGS_URL` (optional), the `embeddings` GitHub release asset (`RagStore.GITHUB_RELEASE_URL`, published by the `Embeddings refresh` workflow after `scripts/rag-eval.mjs` passes; the repo is public since 2026-09-05 so no credential is needed; if it goes private again, `CHROMIA_EMBEDDINGS_TOKEN` / `/etc/secrets/CHROMIA_EMBEDDINGS_TOKEN` / `GITHUB_TOKEN` routes the download through the releases API, else 404 and fall-through), then the GitLab package. The store is streamed in (`EmbeddingStoreJson`), never read whole; it needs a 200-224 MB heap. Generation is a separate path (`RagStore(loadFromRegistry = false)`): `./gradlew :app:generateEmbeddings` (persist + upload) or `:app:generateEmbeddingsNoUpload` (persist only to `app/build/embeddings.json`).
...
- The hosted Render SSE service is **retired (suspended 2026-09-05)**; the product is the local install (stdio via `npx chromia-mcp` / `scripts/install.mjs`, or the fat JAR; local SSE on `127.0.0.1:3001`, which is also `scripts/e2e-sweep.mjs`'s default). Do not add hosted-server plumbing or point docs at a hosted URL. `render.yaml` and the Dockerfile are kept only for the `claude-code-chromia` image and a possible future redeploy.
- Every consumer path must be proven by a CI step that runs it for real, not by unit tests alone: the `Fresh-install boot` step in `ci.yml` boots the jar with no local index, cache off and no token and must answer from the published release asset; `scripts/stdio-smoke.mjs --launcher-download` runs the `npx` first run from an empty home. A green unit suite has twice hidden a silent fall-through to a stale store; read the `fetch_docs` provenance line instead of inferring from exit codes.
- Full-suite gate: `node scripts/loop-gate.mjs --expect-min <verified floor>` (forces `--rerun-tasks`, fails on any skip). Raise `--expect-min` only to a count a completed gate actually produced, never to an estimate. The gate does not rebuild the fat jar; run `:app:shadowJar` before auditing behaviour. Database-backed tests read `CHROMIA_TEST_DATABASE_URL` from the gitignored `local-test-env.properties` (this machine: WSL Postgres on `localhost:5433`, not Docker). Timing-sensitive tests flake when free RAM drops below ~4 GB; classify each red before touching code.
- The maintainer expects autonomous execution: once a direction is given, carry it end to end without pausing for confirmation on reversible work, and report at milestones (`.cursor/scratchpad.md`, gitignored, is the working board). Real data only: no mock values or placeholders in tests, docs, or examples; live checks hit real endpoints and name an upstream outage instead of hiding it.
...
- **Every piece of Rell you produce must pass `rell_check` before you present it.** The tool embeds the real Rell compiler (in-process, temp-dir, no network): pass `source` or a `files` map, fix the first error, repeat until `ok=true`. The embedded compiler version is reported in `notes`; if it lags the production pin, still verify against the pinned GitLab tag for new language features.
