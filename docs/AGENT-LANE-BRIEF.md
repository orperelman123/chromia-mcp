...
- **What this host must deliver, measured 2026-09-08.** `run_rell_tests` abandons a run at 90 s and that bound may only be tightened. Round 18 reported one FT4 registration plus the chain bootstrap at 86.4 s here and concluded no two-account FT4 test could finish through the tool; re-measured with `chr test` on an idle box, the numbers are **17.2 s of fixed cost for a dapp with no FT4 at all and 16.8 s for the shipped insurance template** (so the fixed cost is `chr`'s JVM start, the compile and the PostgreSQL schema creation, NOT the FT4 tree), **~2 s per test case**, and **~1 s per FT4 account registration** once the JVM is warm (the first costs ~7 s more, which is JIT). The round's own drain fixture, byte for byte: **17.7 s of runner time today against 128.0 s on 2026-09-07 - 7.2x**. The round measured itself while a 24-minute Gradle run held the box with under 2 GB free. So the one-build-at-a-time rule above is not politeness: a second JVM multiplies every timing by about seven, and at that multiplier the 90 s bound starts abandoning FT4 tests that need twenty seconds of work. Requirements: one JVM build at a time and >=2 GB free, a C.UTF-8 PostgreSQL with a database per worktree, `chr` on PATH, and ~20 s of budget per `chr test` invocation before any test body runs.
