...
- **Everything must be real.** No stubs, no tests that pass without exercising what they name, no claim the code cannot back. A red that is true beats a green that is not; a branch is left failing rather than softened. Two tests enforce this mechanically and both fail on a new violation: `NoTestDoublesTest` requires there to be NONE - not of our own code and not of a third party either - and it proves it STRUCTURALLY, off the compiled classes: a seam in the supertype closure, a call to a JDK factory that manufactures an implementation, or an invokedynamic over a substitutable single-method interface. A seam is every interface or abstract class production's own bytecode names (round 19: a substitute for a LIBRARY collaborator is a double too), and the trees it reads are the test task's own runtime classpath minus the dependency jars and production's output, so a new source set cannot appear unscanned. The ledger that used to allow a double with a named live counterpart was removed on 2026-09-07, after pointing the explorer tools at the real explorer showed four of them had never worked and every one had a green unit test, and `AssumptionLedgerTest` does the same for every site that can skip.
