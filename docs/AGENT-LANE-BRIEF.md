...
- **Everything must be real.** No stubs, no tests that pass without exercising what they name, no claim the code cannot back. A red that is true beats a green that is not; a branch is left failing rather than softened. Two tests enforce this mechanically and both fail on a new violation: `MockLedgerTest` scans the test sources for doubles and requires each to name what third party it stands in for and which live check covers the same path (a double of OUR OWN code is refused outright), and `AssumptionLedgerTest` does the same for every site that can skip.
